...
function isValidVideoUrl(url) {
    try {
        ...
        // Just check if it's a valid URL, no restrictions on domain
        return true;
    } catch (e) {
        return false;
    }
...
const openUploadModal = () => {
    ...
    // Reset form
    // document.getElementById('upload-form').reset(); // Removed to preserve saved data
    
    // Reset seasons data
    // seasons = []; // Removed to preserve saved data
    // seasonCounter = 0; // Removed to preserve saved data
    // document.getElementById('seasons-list').innerHTML = ''; // Removed to preserve saved data
...
const closeUploadModal = () => {
    ...
    // Don't reset form to preserve data
...
const addEpisode = (seasonId) => {
    ...
    episodeElement.innerHTML = `
        ...
        <div class="episode-actions">
            <input type="url" placeholder="Episode URL" style="width: 150px; padding: 5px; margin-right: 5px;" onchange="updateEpisodeUrl('${episodeId}', this.value)">
            <input type="url" placeholder="Download URL" style="width: 150px; padding: 5px; margin-right: 10px;" onchange="updateEpisodeDownloadUrl('${episodeId}', this.value)">
        ...
        if (formData.seasons && formData.seasons.length > 0) {
            ...
            seasons.forEach(season => {
                ...
                seasonHeader.innerHTML = `
                    ...
                    <div class="season-actions">
                        <button type="button" class="season-btn" onclick="addEpisode('${season.id}')">Add Episode</button>
                        <button type="button" class="season-btn remove" onclick="removeSeason('${season.id}')">Remove</button>
                    </div>
                `;
                
                const episodesContainer = document.createElement('div');
                episodesContainer.className = 'episodes-container';
                episodesContainer.id = `${season.id}-episodes`;
                ...
                season.episodes.forEach(episode => {
                    ...
                    episodeElement.innerHTML = `
                        <div class="episode-info">
                            ...
                            <div class="episode-title">
                                <input type="text" placeholder="Episode title" value="${episode.title}" onchange="updateEpisodeTitle('${episode.id}', this.value)">
                            </div>
                        </div>
                        <div class="episode-actions">
                            <input type="url" placeholder="Episode URL" style="width: 150px; padding: 5px; margin-right: 5px;" value="${episode.url || ''}" onchange="updateEpisodeUrl('${episode.id}', this.value)">
                            <input type="url" placeholder="Download URL" style="width: 150px; padding: 5px; margin-right: 10px;" value="${episode.downloadUrl || ''}" onchange="updateEpisodeDownloadUrl('${episode.id}', this.value)">
                            <button type="button" class="episode-btn remove" onclick="removeEpisode('${season.id}', '${episode.id}')">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    `;
            ...
            // Show series fields if type is series or anime
            toggleSeriesFields();
        }
    }
}
...
const openVideoNav = async (videoUrl, videoId, item) => {
    ...
    // For other video sources, use as is
    else {
    ...
    if (item.downloadUrl) {
        // Show download button for videos with download URL
        downloadBtn.style.display = 'flex';
        downloadBtn.onclick = () => downloadVideo(item.downloadUrl);
    } else {
        // Hide download button for videos without download URL
    ...
    if (item && item.isSeries && item.seasons) {
        ...
        item.seasons[item.currentSeason].episodes.forEach((episode, index) => {
            ...
            episodeBtn.onclick = () => {
                ...
                // Update download button if download URL is available
                const downloadBtn = document.getElementById('video-download-btn');
...
const goToSeason = (seasonIndex) => {
    ...
    if (currentSeason.episodes.length > 0) {
        ...
        // Update download button if download URL is available
        const downloadBtn = document.getElementById('video-download-btn');
        if (currentSeason.episodes[0].downloadUrl) {
            downloadBtn.style.display = 'flex';
            downloadBtn.onclick = () => downloadVideo(currentSeason.episodes[0].downloadUrl);
        } else {
            downloadBtn.style.display = 'none';
        }
    }
...
// Improved notification system - Updated to display in the middle of the app
function showNotification(message, type = 'info') {
    const notification = document.createElement('div');
    notification.className = `notification ${type}`;
    notification.textContent = message;
    
    // Position fixed at center of screen
...
window.addEventListener('load', () => {
    ...
    document.getElementById('upload-form').addEventListener('input', saveFormDataToLocalStorage);
});
